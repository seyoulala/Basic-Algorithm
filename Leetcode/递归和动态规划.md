...
2. 当k=2时，此时我们有两颗鸡蛋。第二颗鸡蛋的使用策略是由第一颗鸡蛋使用的策略决定的。一旦第一颗鸡蛋碎了，假设在第k层碎了，那么我们只能从k层以下一层层的重新去找。所以第一颗鸡蛋使用的策略很重要。比如我们在扔第一颗鸡蛋的时候每隔10层扔一次。那么在这种情况下最坏的结果就是我在100层的时候碎了，然后我需要从91层开始最多扔9次来确定这个值。因此最小次数就是19。
...
题目要求我们输出的是字符串b能否完全匹配上字符串a，对于两个字符串的问题，一般都是使用双指针。如果没有`.`和`*`这两个字符，
...
```python
	def isMatch(self,text,pattern):
        ##递归函数的作用
        ##如果当前两个字符串的开头相同，那么去掉开头字符串的两个字符串开头还相同么？
        def dp(i,j):
            if i==len(text):
                return  j==len(pattern)
            first = text[i]==pattern[j]
            return first and dp(i+1,j+1)
        return  dp(0,0)
```

让两个指针同时往后走。递归终止条件为其中一个字符串走到尾了。但是现在多了`.`和`*`,`.`字符比较好处理，因为其能匹配任意一个字符我们将`first=text[i]==pattern[j]`改为`first=text[i]==pattern[j] or pattern[j]=='.'`就可以处理了。关键是如何处理`*`这个字符。`*`表示匹配`*`前面的字符0次或者多次，那么如果需要匹配`*`前面的字符的话，那么到底需要匹配多少次呢？我们可以将这个问题交给递归函数来处理，记住我们递归函数的作用是判断字符串a从i位置开始能否和字符串b从j位置开始完全匹配。如果我们在字符串b中发现有`*`字符了，那么如果`*`字符串前面的字符就和a中的字符只有匹配和不匹配两中情况。

比如`s='aab',p='c*a*b',i=0,j=0,我们发现j后面的字符是*号，那么此时我们就需要判断了，c这个字符要不要匹配。如果不匹配s[i]!=p[j]，那么j就要跳到*后面的a也就是j=j+2，然后继续和i=0处的字符开始匹配。此时我们发现s[i]=p[j]了，那么a这个字符在之后还要不要匹配呢？我们可以让i往后移动，j不动然后继续去比较。那么问题就变成了s[i]=p[j]，那么dp(i+1,j)是否匹配？`，因此我们写出下面这个函数：

注意一些边界条件，如我们在比较i，j两个位置的字符是否相同时，都要去判断一下j后面的字符是`*`号，
...
```python
    def isMatch(self,text,pattern):
        ...
        def dp(i,j):
            ...
            if j==len(pattern):
                return i==len(text)
            #如果还没有匹配完，那么当前i和j是否能匹配上？
            if i<len(text):
                first = (text[i] == pattern[j]) or pattern[j] == '.'
            else:
                first=False
            #是否发现*通配符,*不会出现在开头。那么*前面的字符是否要出现？
            if j<len(pattern)-1  and pattern[j+1]=='*':
                return dp(i,j+2) or  (first and dp(i+1,j))
            else:
                return  dp(i+1,j+1) and first
        return  dp(0,0)
```

<img src="/Users/eason/Library/Application%20Support/typora-user-images/image-20201225215717667.png" alt="image-20201225215717667" style="zoom:50%;" />



同时，对于自顶向下的递归，可以使用备忘录来处理重叠子问题。
...
class Solution:
    def isMatch(self, s: str, p: str) -> bool:
        ...
        def dp(i,j):
            ...
            #开头字符是否匹配
            if i<len(s):
                first = s[i]==p[j] or p[j]=='.'
            else:
                first=False
            #是否p中有*字符
            if j<len(p)-1 and p[j+1]=='*':
                mem[(i,j)] = dp(i,j+2) or (first and dp(i+1,j))
            else:
                mem[(i,j)] = first and dp(i+1,j+1)
            return mem[(i,j)]
        return  dp(0,0)
```
...
现在题目要求我们获得硬币的最大数量，这种求最值的问题一般都可以使用动态规划来解。但是如果使用暴力求解呢？我们就需要列出所有可能的戳气球的顺序，然后求这些顺序中那种能获得最大的硬币数。如果枚举出所有的顺序，其实就是一个全排列的问题。根据框架写出如下代码:
  ...
  def __init__(self):
    self.ress=-999
      def qiqiu(self,nums):
        ...
        # res = []
        # a=[]
        def __dfs(nums,index,temp,used):
            if index==len(nums):
                self.ress = max(self.ress,temp)
                return

            for  i in range(len(nums.copy())):
                if used[i]:
                    left = 1
                    ...
                    for t in range(i+1,len(nums)):
                        if used[t]:
                            right = nums[t]
                            break
                    sum_=nums[i]*left*right
                    temp+=sum_
                    used[i] = False
                    __dfs(nums,index+1,temp,used)
                    used[i]=True
                    temp-=sum_
        __dfs(nums, 0, 0, used)
        return self.ress
...
但是暴力枚举的时间复杂度很高，基本时间复杂度是阶乘级别。同时我们在枚举的过程中发现有大量的重叠子问题。

**动态规划来求解**:

在使用回溯算法枚举所有可能的方案时，我们发现有大量的重叠子问题，我们可以备忘录来进行优化，同时我们也能使用动态规划来处理。对于区间问题，或两个字符串问题。我们基本都是使用二维的dp表来求解。这里也一样，题目要求我们求出戳破区间[i,j]之间所有的气球所能获得最大收益，那么我们就可以定义`dp[i][j]`表示戳破区间[i,j]内所有气球能获得最大的收益。那么我们第一个气球戳哪一个呢？我们可以进行枚举，假设是第k个。那么在第k个气球这里，我们将问题分成了两个子问题，`dp[i][k],dp[k,j]`，只要我们知道了`dp[i][k]和dp[k][j]`这两个区间戳破气球的最大收益，我们就可以知道原问题`[i,j]`区间的解了，也就是`dp[i][j]=dp[i][k]+num[k]*num[i]*num[j]+dp[k][j]`,但是右题目知道，戳破k后K-1个气球就和k+1个气球相邻了。`dp[i][k]和dp[k][j]`这两个子问题会互相影响，因为当你先戳左边的k-1个气球，此时k+1气球就和k-2相邻了。但是你选先戳破k+1个气球，那么k-1就和k+2相邻。

因此为了消除子问题互相独立的问题，我们定义`dp[i][j]`为戳破（i,j）之间所有气球能够获得最大的收益，注意是开区间，也就是i和j都不能被戳破。同时定义k为区间中最后一个被戳破的气球，这样我就能直接计算戳破该气球能够获取的奖励，也就是`num[k]*num[i]*num[j]`

根据我们要求解的dp[0][n+1],我们能知道遍历顺序

<img src="/Users/eason/Library/Application%20Support/typora-user-images/image-20201229201818628.png" alt="image-20201229201818628" style="zoom:33%;" />
...
```python
    def maxCoins(self, nums: List[int]) -> int:
        if len(nums)==0:
            return 0
        #定义dp数组
        # dp[i][j]表示戳破(i,j)之间所有气球时能够得到的最大的金币,(i,j)为开区间
        #最后要求的状态为dp[0][n+1]
...
题目告诉我们，石子的总数是奇数，所以没有平局。同时玩家每次可以从行的开始或结束的地方取走整堆石头。最后谁手中的石头最多，那么就能够获得胜利。

还是老问题，首先暴力解就是枚举出先手人的所有可能的取石头的方案，因为两人都发挥出了最佳的水平，所以取方案中取石头最大的一组方案作为先手人的方案，此时剩余的石头就是后手人能够取得数量，我们比较两个值就可以判断先手的人是否能够取胜。通过dfs我们可以枚举出所有可能的方案。
    ...
    def stoneGame(self, piles: List[int]) -> bool:
        ...
        def __dfs(piles,index,temp):
            ...
            for i in [0,-1]:
                ...
                a = piles.pop(i)
                __dfs(piles,index+1,temp)
                temp-=a
                piles.insert(i,a)
        __dfs(piles, 0, 0)
        total = sum(piles)
        res = True if t>(total-t) else  False
        return  res
```
...
和其它题目很类似，其实就是类似问我们在`[i,....j]`这个区间里面，先手的人能够获得的最大的分数，这和戳气球的问题是很相似的，所以我们首先可以这样定义dp表:`dp[i][j]`表示在i，j这个区间内能够能够取得的最大分数。

但是题目告诉我们，两个人是轮流着拿石头的。这意味着，如果我在[i,j]区间在i位置拿了，那么对于区间[i+1,j],那么我就是后手拿了。

那么此时，我们就需要两张dp表，一张是表示我先手哪能够拿到的最大值，另外一张是后手拿能够拿到的最大值。同时这两个dp表的大小都是一样的，所以我们可以将两个表变为一个表，但是在一个位置`dp[i][j]`同时存两个值，一个是先手哪能够拿到的最大值，一个是后手拿能够获得最大值，这里我们使用`dp[i][j][0]`表示先手拿能取的最大值`dp[i][j][1]`能取得最大值

*状态转移方程* :`dp[i][j][0]=max(nums[i]+dp[i-1][j][1],nums[j]+dp[i][j-1][1])`,`dp[i][j][1] = max(dp[i+1][j][0],dp[i][j-1][0])`

比如说我对于题目中[5,3,4,5]，如果我在0这个位置先先拿了，那么在[3,4,5]这个区间中我相当于另外一个人就是后手拿了。如果我在index=3这个位置拿了，那么相当于[5,3,4]这个区间我就是后手拿的。因为题目中说了两个人都是发挥的最佳，那么我们就取其中的最大值最为先手的人能拿到的最大值。

最后我们要求解的就是`dp[0][n-1]`这个位置的值。

<img src="/Users/eason/Library/Application%20Support/typora-user-images/image-20210102205933046.png" alt="image-20210102205933046" style="zoom:33%;" />



依据最后要求解的状态值和状态转移方程，我们需要斜着遍历这个二维表格。
...
class Solution(object):
    def stoneGame(self, piles: List[int]) -> bool:
        length = len(piles)
        #定义dp数组
        dp =[[[0] * 2 for i in range(length)] for i in range(length)]
        for i in range(length):
            dp[i][i][0]=piles[i]
            dp[i][i][1]=0
        for l in range(1,length):
            ...
            for i in range(0,length-l):
                ...
                #左边大于右边，那么先手选左边，后选相当于[i+1，j]这个区间的先选
                if left>right:
                    dp[i][j][0] = left
                    dp[i][j][1] =dp[i+1][j][0]
                else:
        ...
        fir = dp[0][length-1][0]
        sec = dp[0][length-1][1]
        return  True if (fir-sec)>0 else  False
```
...
题目告诉我们小偷不能偷相邻房间的现金。同时我们知道每个房间对于小偷来说只有两种情况，要么进去偷，那么不进去偷，如果进去偷了，那么小偷下一次只能去下下家偷，如果不进去，那么小偷下次能去下一家偷。很容易写出递归的解法来求解
...
class Solution:
    def rob1(self, nums: List[int]) -> int:
        ...
        def dp(index,money):
            if index>=length:
                return money
            return max(dp(index+1,money),dp(index+2,money+nums[index]))
        return dp(0,0)
```

通过画出递归树，我们发现会有很多重叠的子问题。对于重叠的子问题可以通过备忘录来处理。这里我们直接跳过，使用自底向上的动态规划去求解。

那么`状态是什么?,选择又是什么？`

根据题目我们可以知道，状态就是小偷现在所处的位置，选择的话就是偷当前房子和不偷当前房子，我们选其中较大的那一种。因此我们定义dp[i]表示偷到第i个房子能偷的最大的金额。

在初始化话的时候dp[0]=0表示还没有房子可以投，dp[1]=nums[0]表示偷到第一家房子能偷到的最大的金额.
...
思路和打家劫舍1是一样的，只不过这里多了一个限制，所有的房屋都围成了一圈，第一个房子和最后一个房子不能同时偷。如果不偷第一个房子，那么问题就变成了在nums[1:]这个数组中能偷的最多的金额。如果第一个房子要偷，那么就变成了在nums[:length-1]这个数组中能偷的最大的金额，那么到底那一种方式能偷的金额最多呢？我们取其中较大的一种就行了。
...
class Solution:
    def rob(self, nums: List[int]) -> int:
        ...
        if length==1:
            return nums[0]
        s1 = rob1(nums[1:length])
        s2 = rob1(nums[:length-1])
        return max(s1,s2)
```
