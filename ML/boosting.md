...
集成算法想要获得性能上的提升，基学习器应该具有独立性，虽然个体学习器做不到独立，但是应该让其尽可能具有差异性。

bagging给出的做法是对数据集的重采样，产生若干个不同的数据子集，然后用这些子集训练出不同的学习器，这样由于训练数据的不同，个体学习器可能就会产生不同的差异。

但实际上，由于训练子集有着相似性以及用的是**同质**学习器，因此训练出来的模型拥有近似相等的bais和variance

由于$E(X_i)=E(\frac{\sum_{X_i}}n)$,所以bagging后的bais和单个子模型的bais接近，一般来说不能显著降低bais。但若是各个模型完全独立，则有$Var(\frac{\sum{X_i}}{n})=\frac{Var(X_i)}{n}$,因此可以显著降低variance,但是如何模型完全相关,那么$Var(\frac{\sum{X_i}}{n})={Var(X_i)}$,此时不会降低variance,bagging得到的子模型具有一定的相关性，属于上述两种情况的中间状态，因此可以在一定程度上降低variance.

注:在这里简单推一下公式.这里假设随机变量$X$有两个状态$X_1,X_2$
...
[GBDT详解](https://blog.csdn.net/weixin_42001089/article/details/84937301)

[XGBOOST详解](https://blog.csdn.net/weixin_42001089/article/details/84965333)
